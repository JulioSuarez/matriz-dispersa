...
bool MatrizV::validar(int numeroMin, int numeroMax) {
	if (numeroMin >= 1 && numeroMin <= numeroMax) {
		return true;
	}
	return false;
	// return (numeroMin >= 1 && numeroMin <= numeroMax);
}
// 3x3
...
// 0 0 0
// 0 0 0
void MatrizV::poner(int f, int c, int valor) { // 1.1.9
	if (validar(f, df) && validar(c, dc)) {
		int lugar = posicion(f, c);
		if (lugar == esLibre) // nt < MAX_MatrizVector
		{
			if (nt < (df * dc)) // nt < MAX_MatrizVector
		...
		cout << "Rango invalido f:" + to_string(f) + " c:" + to_string(c) + "\n";
	}
}
